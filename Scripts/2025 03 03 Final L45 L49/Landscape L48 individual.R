...
if (file.exists("dataset/L48x_gmt.rds")) {
  # Cargar gmt_data del archivo
  gmt_data <- readRDS(file = "dataset/L48x_gmt.rds")
} else {
  ...
  saveRDS(gmt_data, file = "dataset/L48x_gmt.rds")
}
...
# do plot 
PlotL48 <- plot_landscapes_from_list(data3js,
                                     titertables_groups,
                                     lndscp_fits,
                                     map,
                                     gmt_data,
...
# Verificar si el archivo existe
if (file.exists("dataset/PlotL48x.rds")) {
  # Cargar gmt_data del archivo
  gmt_data <- readRDS(file = "dataset/PlotL48x.rds")
} else {
  # Crear gmt_data utilizando la función si el archivo no existe
  PlotL48 <- plot_landscapes_from_list(data3js, titertables_groups, lndscp_fits, map, gmt_data, 
                                       agNames(map), agNames(map), lndscp_colors = sr_colors,
                                       show_gmts = FALSE)
  # Guardar gmt_data en un archivo .rds para futuros usos
  saveRDS(PlotL48, file = "dataset/PlotL48x.rds")
  message("El archivo 'PlotL48x.rds' no se encontró. Se ha creado y guardado un nuevo archivo.")
}
...
result <- sapply(PlotL48[["plot"]], function(elem) {
...
calcular_volumen <- function(index) {
  x <- PlotL48[["plot"]][[unique_indices[index]]][["x"]]
  y <- PlotL48[["plot"]][[unique_indices[index]]][["y"]]
  z <- PlotL48[["plot"]][[unique_indices[index]]][["z"]]
...
for (i in unique_indices) {
  ...
  z_1_1 <- PlotL48[["plot"]][[i]][["z"]][1, 1]
  z_1_10 <- PlotL48[["plot"]][[i]][["z"]][1, 10]
  z_13_1 <- PlotL48[["plot"]][[i]][["z"]][13, 1]
  z_13_10 <- PlotL48[["plot"]][[i]][["z"]][13, 10]
...
if (nrow(dfL48) == nrow(z_values_df)) {
  # Unir ambos data.frames por las columnas
  dfL48 <- cbind(dfL48, z_values_df)
...
ggplot(dfL48, aes(x = factor(1), y = z_1_1/10)) + 
...
ggplot(dfL48, aes(x = factor(1), y = z_1_10/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
...
ggplot(dfL48, aes(x = factor(1), y = z_13_1/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()
...
ggplot(dfL48, aes(x = factor(1), y = z_13_10/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()
...
PlotL48[["plot"]][[238]][["x"]]
PlotL48[["plot"]][[238]][["y"]]
PlotL48[["plot"]][[11460]][["x"]]
PlotL48[["plot"]][[11460]][["y"]]
