...
if (file.exists("dataset/L45x_gmt.rds")) {
  # Cargar gmt_data del archivo
  gmt_data <- readRDS(file = "dataset/L45x_gmt.rds")
} else {
  ...
  saveRDS(gmt_data, file = "dataset/L45x_gmt.rds")
}
...
# do plot 
PlotL45 <- plot_landscapes_from_list(data3js,
                                     titertables_groups,
                                     lndscp_fits,
                                     map,
                                     gmt_data,
...
# Verificar si el archivo existe
if (file.exists("dataset/PlotL45x.rds")) {
  # Cargar gmt_data del archivo
  gmt_data <- readRDS(file = "dataset/PlotL45x.rds")
} else {
  # Crear gmt_data utilizando la función si el archivo no existe
  PlotL45 <- plot_landscapes_from_list(data3js, titertables_groups, lndscp_fits, map, gmt_data, 
                                       agNames(map), agNames(map), lndscp_colors = sr_colors,
                                       show_gmts = FALSE)
  # Guardar gmt_data en un archivo .rds para futuros usos
  saveRDS(PlotL45, file = "dataset/PlotL45x.rds")
  message("El archivo 'PlotL45x.rds' no se encontró. Se ha creado y guardado un nuevo archivo.")
}
...
result <- sapply(PlotL45[["plot"]], function(elem) {
...
calcular_volumen <- function(index) {
  x <- PlotL45[["plot"]][[unique_indices[index]]][["x"]]
  y <- PlotL45[["plot"]][[unique_indices[index]]][["y"]]
  z <- PlotL45[["plot"]][[unique_indices[index]]][["z"]]
...
for (i in unique_indices) {
  ...
  z_1_1 <- PlotL45[["plot"]][[i]][["z"]][1, 1]
  z_1_10 <- PlotL45[["plot"]][[i]][["z"]][1, 10]
  z_13_1 <- PlotL45[["plot"]][[i]][["z"]][13, 1]
  z_13_10 <- PlotL45[["plot"]][[i]][["z"]][13, 10]
...
if (nrow(dfL45) == nrow(z_values_df)) {
  # Unir ambos data.frames por las columnas
  dfL45 <- cbind(dfL45, z_values_df)
...
ggplot(dfL45, aes(x = factor(1), y = z_1_1/10)) + 
...
ggplot(dfL45, aes(x = factor(1), y = z_1_10/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
...
ggplot(dfL45, aes(x = factor(1), y = z_13_1/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()
...
ggplot(dfL45, aes(x = factor(1), y = z_13_10/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()
...
PlotL45[["plot"]][[238]][["x"]]
PlotL45[["plot"]][[238]][["y"]]
PlotL45[["plot"]][[11460]][["x"]]
PlotL45[["plot"]][[11460]][["y"]]
