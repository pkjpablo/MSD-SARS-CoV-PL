...
db45 <- merge(db45, dfL45.2[, c("id", "volumen_percent", "slope_gam")],by.x =  "idnova.x", by.y = "id", all.x = TRUE)


db45$volumen100
db45$volumen_percent
db45$razao_L45
...
m45a <- lm(volumen_percent~ 
            cind_sexo +
            categoria_edad  , data = db45)

tab_model(m45a,show.aic = T)

############ Modelo L46
db46 <- merge(db46, dfL46.2[, c("id", "volumen_percent", "slope_gam")],by.x =  "idnova.x", by.y = "id", all.x = TRUE)


m46 <- lm(volumen100 ~  cind_sexo+
            categoria_edad+razao_L45+factor(vac_cat.L46), data = db46)

tab_model(m46)

db46$volumen_percent
m46 <- lm(volumen_percent ~  cind_sexo+
            categoria_edad+razao_L45+factor(vac_cat.L46), data = db46)
...
db46 <- db46 %>%
  mutate(
    grupo3 = case_when(
      grupo2.x %in% c("RE", "RE or SV", "SV") ~ "3. Dob_expo",
      grupo2.x %in% c("SS1", "SS2", "V", "SS2 or V") ~ "2. Single_expo",
      grupo2.x == "NVSN" ~ "1. NVSN",
