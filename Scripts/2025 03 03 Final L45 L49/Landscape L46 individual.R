...
if (file.exists("dataset/L46x_gmt.rds")) {
  # Cargar gmt_data del archivo
  gmt_data <- readRDS(file = "dataset/L46x_gmt.rds")
} else {
  ...
  saveRDS(gmt_data, file = "dataset/L46x_gmt.rds")
}
...
# do plot 
PlotL46 <- plot_landscapes_from_list(data3js,
                                     titertables_groups,
                                     lndscp_fits,
                                     map,
                                     gmt_data,
...
# Verificar si el archivo existe
if (file.exists("dataset/PlotL46x.rds")) {
  # Cargar gmt_data del archivo
  gmt_data <- readRDS(file = "dataset/PlotL46x.rds")
} else {
  # Crear gmt_data utilizando la función si el archivo no existe
  PlotL46 <- plot_landscapes_from_list(data3js, titertables_groups, lndscp_fits, map, gmt_data, 
                                       agNames(map), agNames(map), lndscp_colors = sr_colors,
                                       show_gmts = FALSE)
  # Guardar gmt_data en un archivo .rds para futuros usos
  saveRDS(PlotL46, file = "dataset/PlotL46x.rds")
  message("El archivo 'PlotL46x.rds' no se encontró. Se ha creado y guardado un nuevo archivo.")
}

PlotL46 <- readRDS("C:/Users/Pablo/OneDrive - FIOCRUZ/Pablo - Fiocruz/1. MSD/1. Landscape clean/MSD/dataset/PlotL46x.rds")
...
result <- sapply(PlotL46[["plot"]], function(elem) {
...
calcular_volumen <- function(index) {
  x <- PlotL46[["plot"]][[unique_indices[index]]][["x"]]
  y <- PlotL46[["plot"]][[unique_indices[index]]][["y"]]
  z <- PlotL46[["plot"]][[unique_indices[index]]][["z"]]
...
for (i in unique_indices) {
  ...
  z_1_1 <- PlotL46[["plot"]][[i]][["z"]][1, 1]
  z_1_10 <- PlotL46[["plot"]][[i]][["z"]][1, 10]
  z_13_1 <- PlotL46[["plot"]][[i]][["z"]][13, 1]
  z_13_10 <- PlotL46[["plot"]][[i]][["z"]][13, 10]
...
if (nrow(dfL46) == nrow(z_values_df)) {
  # Unir ambos data.frames por las columnas
  dfL46 <- cbind(dfL46, z_values_df)
...
# Gráfico para z_1_1, escalado y con rango ajustado (iz) fre
ggplot(dfL46, aes(x = factor(1), y = z_1_1/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()

# Gráfico para z_1_10, escalado y con rango ajustado (atras)   iz
ggplot(dfL46, aes(x = factor(1), y = z_1_10/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()

# Gráfico para z_13_1, escalado y con rango ajustado (frente) de
ggplot(dfL46, aes(x = factor(1), y = z_13_1/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()

# Gráfico para z_13_10, escalado y con rango ajustado (derecha) tra
ggplot(dfL46, aes(x = factor(1), y = z_13_10/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()
...
PlotL46[["plot"]][[238]][["x"]]
PlotL46[["plot"]][[238]][["y"]]
PlotL46[["plot"]][[11460]][["x"]]
PlotL46[["plot"]][[11460]][["y"]]
