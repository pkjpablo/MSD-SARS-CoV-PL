...
if (file.exists("dataset/L47x_gmt.rds")) {
  # Cargar gmt_data del archivo
  gmt_data <- readRDS(file = "dataset/L47x_gmt.rds")
} else {
  ...
  saveRDS(gmt_data, file = "dataset/L47x_gmt.rds")
}
...
# do plot 
PlotL47 <- plot_landscapes_from_list(data3js,
                                     titertables_groups,
                                     lndscp_fits,
                                     map,
                                     gmt_data,
...
# Verificar si el archivo existe
if (file.exists("dataset/PlotL47x.rds")) {
  # Cargar gmt_data del archivo
  gmt_data <- readRDS(file = "dataset/PlotL47x.rds")
} else {
  # Crear gmt_data utilizando la función si el archivo no existe
  PlotL47 <- plot_landscapes_from_list(data3js, titertables_groups, lndscp_fits, map, gmt_data, 
                                       agNames(map), agNames(map), lndscp_colors = sr_colors,
                                       show_gmts = FALSE)
  # Guardar gmt_data en un archivo .rds para futuros usos
  saveRDS(PlotL47, file = "dataset/PlotL47x.rds")
  message("El archivo 'PlotL47x.rds' no se encontró. Se ha creado y guardado un nuevo archivo.")
}
...
result <- sapply(PlotL47[["plot"]], function(elem) {
...
calcular_volumen <- function(index) {
  x <- PlotL47[["plot"]][[unique_indices[index]]][["x"]]
  y <- PlotL47[["plot"]][[unique_indices[index]]][["y"]]
  z <- PlotL47[["plot"]][[unique_indices[index]]][["z"]]
...
for (i in unique_indices) {
  ...
  z_1_1 <- PlotL47[["plot"]][[i]][["z"]][1, 1]
  z_1_10 <- PlotL47[["plot"]][[i]][["z"]][1, 10]
  z_13_1 <- PlotL47[["plot"]][[i]][["z"]][13, 1]
  z_13_10 <- PlotL47[["plot"]][[i]][["z"]][13, 10]
...
if (nrow(dfL47) == nrow(z_values_df)) {
  # Unir ambos data.frames por las columnas
  dfL47 <- cbind(dfL47, z_values_df)
...
ggplot(dfL47, aes(x = factor(1), y = z_1_1/10)) + 
...
ggplot(dfL47, aes(x = factor(1), y = z_1_10/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
...
ggplot(dfL47, aes(x = factor(1), y = z_13_1/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()
...
ggplot(dfL47, aes(x = factor(1), y = z_13_10/10)) + 
  geom_violin(fill = "skyblue", color = "darkblue") +
  labs(title = "", x = "", y = "") +
  coord_cartesian(ylim = c(-0.5, 1)) +  # Limitar el rango del eje Y
  theme_minimal()+ theme_classic()
...
PlotL47[["plot"]][[238]][["x"]]
PlotL47[["plot"]][[238]][["y"]]
PlotL47[["plot"]][[11460]][["x"]]
PlotL47[["plot"]][[11460]][["y"]]
